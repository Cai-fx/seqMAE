...
class scb_TF(nn.Module):
    ...
    @nn.compact
    def __call__(self, seq, rna, atac_dpth, train, mlp_only=False):
        n_gene = rna.shape[-1]
...
class Model(BaseModel):
    ...
    def create_model(self):
        self.model = scb_TF(**self.model_config)
    ...
    def create_batch(self, perm, ds: dict) -> dict:
        perm_acc, _ = perm
        batch = {'x_acc': ds['x_acc'][perm_acc,:], 'rna': ds['rna'], 
    ...
    def print_epoch_metrics(self, epoch_metrics_np, t_start, t_stop):
        ...
        example:
            print('epoch: %d, loss: %.4f, time_elapsed %.0fs' % (
                ...
                t_stop-t_start))
        '''
        print('epoch: %d, l: %.4f, l_val: %.4f, l_acc: %.4f, rna_l: %.4f, val_l_acc: %.4f, val_l_rna: %.4f, time_elapsed %.0fs' % (
                self.epoch,
                epoch_metrics_np['l'], 
    ...
    def pred_y(self, n_batch=1, ds=None, static_args=flax.core.freeze({}), save_self=True, return_res=False):
        out_acc, out_rna = np.zeros_like(ds['y_acc']), np.zeros_like(ds['rna'])
        ...
        for i in tqdm(range(n_cyc)):
            ...
            out_acc[perm, :], out_rna = self.apply_model( 
                                        self.state,
                                        ds_batched,
                                        static_args
                                        )
        ...
        if save_self:
            self.y_pred = {'acc': out_acc, 'rna': out_rna} 
        if return_res:
